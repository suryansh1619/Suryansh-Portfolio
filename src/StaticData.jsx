const StaticData = {
    homeData: [
        {
            firstName: "Suryansh",
            lastName: "Sahu",
            description: "Hi, I'm Suryansh Sahu. I'm passionate about Web Development and Data Visualization and excited about the opportunities and challenges that lie ahead. I'm very dedicated to my work and always eager to learn and grow",
            buttonText: "Say Hello"
        }
    ],
    homeSocial: [
        {
            name: "linkedin",
            link: "https://www.linkedin.com/in/suryansh-sahu-b86213258"
        },
        {
            name: "envelope",
            link: ""
        },
        {
            name: "github",
            link: "https://github.com/suryansh1619"
        }
    ],
    about: [
        {
            description: "I am Suryansh Sahu, currently pursuing a Bachelor of Technology in Computer Science and Engineering at the Indian Institute of Information Technology Nagpur. I completed my 10th grade at Sri Sathya Sai Vidya Vihar, GAIL Vijaipur, and followed it with my 12th grade from the same institution, focusing on Physics, Chemistry, and Mathematics. I have always been passionate about technology and its potential to create impactful solutions. At IIIT Nagpur, I have developed a keen interest in full-stack development, web design, and data visualization.",
            buttonText: "Download CV"
        }
    ],
    aboutInfo: [
        {
            title1: "Experience",
            subtitle1: "0 years Working",
            title2: "Completed",
            subtitle2: "5+ Projects"
        }
    ],
    footer: [
        {
            name: "instagram",
            link: "https://www.instagram.com/suryansh__829?igsh=cjU4enVxaHM1eGpv"
        },
        {
            name: "twitter",
            link: "https://x.com/Suryansh1619?t=26PJTqSl41n2kgZoO96sHg&s=08"
        },
        {
            name: "telegram",
            link: "http://t.me/suryansh_1619"
        },
        {
            name: "linkedin",
            link: "https://www.linkedin.com/in/suryansh-sahu-b86213258"
        },
        {
            name: "github",
            link: "https://github.com/suryansh1619"
        }
    ],
    projectsProject: [
        {
            imgUrl: "https://media.istockphoto.com/id/1411195926/photo/project-manager-working-on-laptop-and-updating-tasks-and-milestones-progress-planning-with.jpg?s=2048x2048&w=is&k=20&c=d--cQ2K441EZ37SZ_w-0h1czZfTGKqcQXdKuvCrpPss=",
            title: "Dog Breed Classifier",
            category: "Machine Learning",
            description: "Developed a CNN-based image classification model in Python using TensorFlow and Keras to classify 120 dog breeds with 92% accuracy, leveraging the pre-trained EfficientNetV2S architecture",
            subtitle1: "Python",
            subtitle2: "TensorFlow",
            subtitle3: "Keras",
            subtitle4: "Scikit-Learn",
            subtitle5: "Matplotlib",
            subtitle6: "Pandas",
            link: "https://github.com/suryansh1619/Dog-Breed-Classifier"
        },
        {
            imgUrl: "https://media.istockphoto.com/id/1411195926/photo/project-manager-working-on-laptop-and-updating-tasks-and-milestones-progress-planning-with.jpg?s=2048x2048&w=is&k=20&c=d--cQ2K441EZ37SZ_w-0h1czZfTGKqcQXdKuvCrpPss=",
            title: "Emotion Analysis",
            category: "Machine Learning",
            description: "Built an NLP-based emotion analysis model using Bidirectional LSTMs, achieving 99.9% accuracy on 50,000+ text samples.",
            subtitle1: "NLTK",
            subtitle2: "Scikit-Learn",
            subtitle3: "TensorFlow",
            subtitle4: "Keras",
            subtitle5: "WordCloud",
            subtitle6: "Matplotlib",
            link: "https://github.com/suryansh1619/Emotion-Analysis"
        },
        {
            imgUrl: "https://media.istockphoto.com/id/1411195926/photo/project-manager-working-on-laptop-and-updating-tasks-and-milestones-progress-planning-with.jpg?s=2048x2048&w=is&k=20&c=d--cQ2K441EZ37SZ_w-0h1czZfTGKqcQXdKuvCrpPss=",
            title: "MoneyMate",
            category: "Web",
            description: "A financial dashboard with JWT-based authentication, enabling real-time tracking of income, expenses, and budgets. Integrated multi-currency support and interactive analytics with Chart.js for financial insights.",
            subtitle1: "ReactJS",
            subtitle2: "NodeJS",
            subtitle3: "Tailwind CSS",
            subtitle4: "PostgreSQL",
            subtitle5: "Express",
            subtitle6: "JWT",
            link: "https://github.com/suryansh1619/MoneyMate"
        },
        {
            imgUrl: "https://media.istockphoto.com/id/1411195926/photo/project-manager-working-on-laptop-and-updating-tasks-and-milestones-progress-planning-with.jpg?s=2048x2048&w=is&k=20&c=d--cQ2K441EZ37SZ_w-0h1czZfTGKqcQXdKuvCrpPss=",
            title: "Himalayan Trekker",
            category: "Web",
            description: "A tourism website exploring 12 districts of Himachal Pradesh, featuring AI-generated content, real-time weather updates, and interactive maps.",
            subtitle1: "HTML",
            subtitle2: "Javascript",
            subtitle3: "Python",
            subtitle4: "Django",
            subtitle5: "OpenAI",
            subtitle6: "SQLite2",
            link: "https://github.com/suryansh1619/Code_For_Good"
        },
        {
            imgUrl: "https://media.istockphoto.com/id/1411195926/photo/project-manager-working-on-laptop-and-updating-tasks-and-milestones-progress-planning-with.jpg?s=2048x2048&w=is&k=20&c=d--cQ2K441EZ37SZ_w-0h1czZfTGKqcQXdKuvCrpPss=",
            title: "Color Picker",
            category: "Web",
            description: "A dynamic color picker tool built with ReactJS, allowing users to select, copy, and generate color palettes effortlessly. Features real-time color preview and smooth UI interactions.",
            subtitle1: "HTML",
            subtitle2: "CSS",
            subtitle3: "JavaScript",
            subtitle4: "ReactJS",
            subtitle5: "Material UI",
            subtitle6: "",
            link: "https://github.com/suryansh1619/Color-Picker"
        },
        {
            imgUrl: "https://media.istockphoto.com/id/1411195926/photo/project-manager-working-on-laptop-and-updating-tasks-and-milestones-progress-planning-with.jpg?s=2048x2048&w=is&k=20&c=d--cQ2K441EZ37SZ_w-0h1czZfTGKqcQXdKuvCrpPss=",
            title: "Portfolio",
            category: "Web",
            description: "A fully responsive and customizable portfolio website built with ReactJS, dynamically rendering content from a static data file. Features smooth UI, theme customization, and an optimized layout.",
            subtitle1: "HTML",
            subtitle2: "CSS",
            subtitle3: "JavaScript",
            subtitle4: "React",
            subtitle5: "Material UI",
            subtitle6: "",
            link: "https://github.com/suryansh1619/Portfolio-main"
        }
    ],
    qualificationEducation: [
        {
            title: "10th",
            subtitle: "Sri Sathya Sai Vidya Vihar",
            calender: "2018-2019"
        },
        {
            title: "12th",
            subtitle: "Sri Sathya Sai Vidya Vihar",
            calender: "2020 - 2021"
        },
        {
            title: "BTech",
            subtitle: "IIIT nagpur",
            calender: "2022-present"
        }
    ],
    qualificationAchievement: [
        {
            title: "Codeforces",
            subtitle: "Rating : 1000",
            calender: "Till 7/24"
        },
        {
            title: "Codechef",
            subtitle: "Rating : 1615",
            calender: "Till 7/24"
        },
        {
            title: "Leetcode",
            subtitle: "Rating : 1833",
            calender: "Till 12/24"
        }
    ],
    skills: [
        {
            title: "HTML",
            level: "Basic",
        },
        {
            title: "CSS",
            level: "Basic",
        },
        {
            title: "JS",
            level: "Basic",
        },
        {
            title: "React JS",
            level: "Basic",
        },
        {
            title: "Node JS",
            level: "Basic",
        },
        {
            title: "C",
            level: "Intermediate",
        },
        {
            title: "C++",
            level: "Intermediate",
        },
        {
            title: "Postgre SQL",
            level: "Basic",
        },
        {
            title: "Python",
            level: "Intermediate",
        },
        {
            title: "MongoDB",
            level: "Intermediate",
        },
        {
            title: "R",
            level: "Basic",
        },
        {
            title: "PHP",
            level: "Basic",
        },
        {
            title: "My SQL",
            level: "Intermediate",
        },
        {
            title: "Numpy",
            level: "Intermediate",
        },
        {
            title:"Pandas",
            level:"Intermediate",
        },
        {
            title: "Matplotlib",
            level: "Intermediate",
        },
        {
            title: "Intermediate",
            level: "Intermediate",
        },
        {
            title: "Scikit-Learn",
            level: "Intermediate",
        },
        {
            title: "TensorFlow",
            level: "Basic",
        }
    ],
    theme: [
        {
            name: 'RED',
            color: 'rgb(217,49,37)',
            selected: false
        },
        {
            name: 'GREEN',
            color: 'rgb(0,161,126)',
            selected: false
        },
        {
            name: 'CYAN',
            color: 'rgb(72,196,217)',
            selected: false
        },
        {
            name: 'PURPLE',
            color: 'rgb(117,54,144)',
            selected: false
        },
        {
            name: 'ORANGE',
            color: 'rgb(227,64,22)',
            selected: false
        },
        {
            name: 'PINK',
            color: 'rgb(224,27,92)',
            selected: false
        },
        {
            name: 'BLUE',
            color: 'rgb(76,178,245)',
            selected: true
        },
        {
            name: 'YELLOW',
            color: 'rgb(228,170,4)',
            selected: false
        }
    ]
};

export default StaticData;